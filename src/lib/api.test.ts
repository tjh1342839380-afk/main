...
describe('callImageApi', () => {
  ...
  it('shows a friendly message for upstream HTML timeout errors', async () => {
    vi.spyOn(globalThis, 'fetch').mockResolvedValue(new Response('<!DOCTYPE html><html><head><title>Timeout</title></head></html>', {
      status: 524,
  ...
  it('adds third-party size aliases for gpt-image-2 custom endpoints', async () => {
    const fetchMock = vi.spyOn(globalThis, 'fetch').mockResolvedValue(new Response(JSON.stringify({
      data: [{ b64_json: 'aW1hZ2U=' }],
    }), {
      status: 200,
    ...
    await callImageApi({
      settings: {
        ...DEFAULT_SETTINGS,
        apiKey: 'test-key',
        baseUrl: 'https://sub2api.toioto.org/v1',
      },
      prompt: 'prompt',
  ...
  it('does not add third-party size aliases for the official OpenAI endpoint', async () => {
    const fetchMock = vi.spyOn(globalThis, 'fetch').mockResolvedValue(new Response(JSON.stringify({
      data: [{ b64_json: 'aW1hZ2U=' }],
    }), {
      status: 200,
    ...
    expect(body.output_size).toBeUndefined()
    expect(body.aspect_ratio).toBeUndefined()
    expect(body.ratio).toBeUndefined()
    expect(body.image_ratio).toBeUndefined()
    expect(body.output_aspect_ratio).toBeUndefined()
    expect(body.aspectRatio).toBeUndefined()
    expect(body.output_ratio).toBeUndefined()
  })
