...
function hasExplicitImageParams(params: TaskParams): boolean {
  return (
    params.size !== 'auto' ||
    params.quality !== 'auto' ||
...
function shouldRequestStreamImages(profile: ApiProfile, params: TaskParams): boolean {
  if (!profile.streamImages) return false
  if (!isOfficialOpenAIBaseUrl(profile.baseUrl) && hasExplicitImageParams(params)) return false
  return true
}

function parseExplicitSize(size: string): { width: number; height: number } | null {
...
function getResolutionLabel(width: number, height: number): '1K' | '2K' | '4K' {
...
function shouldSendCompatibleSizeAliases(profile: ApiProfile, params: TaskParams): boolean {
  if (isOfficialOpenAIBaseUrl(profile.baseUrl)) return false
  return profile.model.trim().toLowerCase().includes('gpt-image-2') && Boolean(parseExplicitSize(params.size))
}

function appendCompatibleSizeAliases(target: Record<string, unknown> | FormData, profile: ApiProfile, params: TaskParams) {
  if (!shouldSendCompatibleSizeAliases(profile, params)) return
  const parsedSize = parseExplicitSize(params.size)
  if (!parsedSize) return
  const resolution = getResolutionLabel(parsedSize.width, parsedSize.height)
  const aspectRatio = formatAspectRatio(parsedSize.width, parsedSize.height)

  if (target instanceof FormData) {
    target.append('width', String(parsedSize.width))
    target.append('height', String(parsedSize.height))
    target.append('resolution', resolution)
    target.append('image_size', params.size)
    target.append('output_size', params.size)
    target.append('aspect_ratio', aspectRatio)
    target.append('ratio', aspectRatio)
    target.append('image_ratio', aspectRatio)
    target.append('output_aspect_ratio', aspectRatio)
    target.append('aspectRatio', aspectRatio)
    target.append('output_ratio', aspectRatio)
    return
  }

  target.width = parsedSize.width
  target.height = parsedSize.height
  target.resolution = resolution
  target.image_size = params.size
  target.output_size = params.size
  target.aspect_ratio = aspectRatio
  target.ratio = aspectRatio
  target.image_ratio = aspectRatio
  target.output_aspect_ratio = aspectRatio
  target.aspectRatio = aspectRatio
  target.output_ratio = aspectRatio
}
...
async function callImagesApiSingle(opts: CallApiOptions, profile: ApiProfile): Promise<CallApiResult> {
  ...
  const useStreamImages = shouldRequestStreamImages(profile, params)
  ...
  try {
    ...
    if (isEdit) {
      ...
      appendCompatibleSizeAliases(formData, profile, params)
      ...
      appendCompatibleSizeAliases(body, profile, params)
    ...
    if (!response.ok) {
      ...
      throw new Error(maybeAppendStreamingHint(errorMessage, response.status, useStreamImages))
    }

    if (useStreamImages && isEventStreamResponse(response)) {
