...
export async function getApiErrorMessage(response: Response): Promise<string> {
  ...
  if (response.status === 524) {
    return '上游服务响应超时。请求已经发送到服务商，但服务商在限定时间内没有返回结果。可以稍后重试，或先降低图片尺寸/质量后再生成。'
  }
  if (response.status === 502 || response.status === 503 || response.status === 504) {
    return `上游服务暂时不可用或响应超时（HTTP ${response.status}）。请稍后重试；如果连续失败，可以检查 API 服务商状态或降低本次生成规格。`
  }
  if (contentType.includes('text/html')) {
    return `API 服务返回了网页错误页（HTTP ${response.status}），不是有效的图片接口响应。请稍后重试，或检查 API URL、服务商状态和网络代理。`
  }
  ...
  try {
    ...
    try {
      ...
      if (/^<!doctype html/i.test(normalizedText) || /^<html[\s>]/i.test(normalizedText)) {
        errorMsg = `API 服务返回了网页错误页（HTTP ${response.status}），不是有效的图片接口响应。请稍后重试，或检查 API URL、服务商状态和网络代理。`
      } else if (normalizedText) {
        errorMsg = normalizedText
      }
    } catch {
      /* ignore */
    }
...
export function pickActualParams(source: unknown): Partial<TaskParams> {
  ...
  if (typeof record.size === 'string' && record.size !== 'auto') actualParams.size = record.size
  if (record.quality === 'low' || record.quality === 'medium' || record.quality === 'high') {
