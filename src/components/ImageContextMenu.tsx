...
import { downloadImageEntriesAsZip, downloadImageIds, formatExportFileTime, getImageZipEntries, saveImageAs } from '../lib/downloadImages'
...
export default function ImageContextMenu() {
  ...
  const getSingleImageFileNameBase = (imageId?: string) => {
    if (imageId) {
      const tasks = useStore.getState().tasks
      const matchedTask = tasks.find(t => t.outputImages?.includes(imageId))
      return matchedTask ? `task-${matchedTask.id}` : `image-${imageId}`
    }

    const timeStr = formatExportFileTime(new Date())
    return `image-${timeStr}`
  }
  ...
  const handleSaveAs = async (e: React.MouseEvent) => {
    ...
    const imageId = menuInfo.imageId
    const src = menuInfo.src
    const fileNameBase = getSingleImageFileNameBase(imageId)
    setMenuInfo(null)

    try {
      const result = await saveImageAs(imageId || src, fileNameBase)
      if (result === 'cancelled') return
      showToast(result === 'saved' ? '已另存为' : '已按默认下载处理', 'success')
    } catch (err) {
      console.error(err)
  ...
  const MENU_HEIGHT = showDownloadAll ? 192 : 160
