...
export default function SettingsModal({ consoleMode = false }: SettingsModalProps) {
  ...
  const apiProxyDynamicTarget = readRuntimeEnv(import.meta.env.VITE_API_PROXY_DYNAMIC_TARGET) === 'true' && !apiProxyConfig
  ...
  const apiProxyDisablesUrl = apiProxyEnabled && !apiProxyDynamicTarget
              ...
              {activeProviderUsesApiUrl && (
                <label className="block">
                  ...
                  <input
                    ...
                    type="text"
                    disabled={apiProxyDisablesUrl}
                    placeholder={activeProfile.provider === 'fal' ? DEFAULT_FAL_BASE_URL : DEFAULT_SETTINGS.baseUrl}
                    className={`w-full rounded-xl border border-gray-200/70 bg-white/60 px-3 py-2.5 text-sm text-gray-700 outline-none transition focus:border-blue-300 dark:border-white/[0.08] dark:bg-white/[0.03] dark:text-gray-200 dark:focus:border-blue-500/50 ${apiProxyDisablesUrl ? 'opacity-50 cursor-not-allowed' : ''}`}
                  />
                  <div data-selectable-text className="mt-1.5 min-h-[22px] flex items-center text-xs text-gray-500 dark:text-gray-500">
                    {apiProxyEnabled && apiProxyDynamicTarget ? (
                      <span className="text-yellow-600 dark:text-yellow-500">已开启代理，请求会经当前站点转发到此 API URL。</span>
                    ) : apiProxyEnabled ? (
              ...
              {apiProxyAvailable && activeProviderIsOpenAICompatible && !activeCustomProviderAsync && (
                <div className="block">
                  ...
                  <div data-selectable-text className="text-xs text-gray-500 dark:text-gray-500">
                    {apiProxyDynamicTarget ? '开启后请求经当前站点转发到上方 API URL，可绕过浏览器跨域限制。' : apiProxyLocked ? '部署端已锁定代理开启，请求经服务器转发到上游 API，上方 URL 设置将失效。' : '开启后请求经服务器转发到上游 API，可绕过浏览器跨域限制，上方 URL 设置将失效。'}
                  </div>
                </div>
              )}
            ...
            {activeTab === 'about' && (
              <div className="flex h-full min-h-[300px] flex-col items-center justify-center px-6 pb-8">
                <div className="flex w-full max-w-[380px] flex-col items-center rounded-[28px] border border-white/60 bg-white/45 px-8 py-10 text-center shadow-[0_20px_60px_rgba(15,23,42,0.08)] ring-1 ring-black/[0.03] backdrop-blur-2xl dark:border-white/[0.08] dark:bg-white/[0.04] dark:shadow-[0_20px_70px_rgba(0,0,0,0.28)] dark:ring-white/[0.05]">
                  ...
                  <p className="mt-8 text-[11px] leading-relaxed text-gray-400 dark:text-gray-500">
                    本站点基于开源项目{' '}
                    <a
                      href="https://github.com/CookSleep/gpt_image_playground"
                      target="_blank"
                      rel="noopener noreferrer"
                      className="font-medium text-gray-500 underline decoration-gray-300/70 underline-offset-4 transition-colors hover:text-gray-700 hover:decoration-gray-500 dark:text-gray-400 dark:decoration-white/20 dark:hover:text-gray-200 dark:hover:decoration-white/40"
                    >
