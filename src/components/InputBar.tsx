...
import { ALL_FAVORITES_COLLECTION_ID, deleteFavoriteCollection, getTaskFavoriteCollectionIds, useStore, submitTask, submitAgentMessage, stopAgentResponse, addImageFromFile, createInputImageFromFile, deleteImageIfUnreferenced, removeMultipleTasks, getCachedImage, ensureImageCached, getActiveAgentRounds, taskMatchesFavoriteScope, taskMatchesFilterStatus, taskMatchesSearchQuery } from '../store'
...
export default function InputBar() {
  ...
  const getVisibleTaskIds = useCallback(() => {
    const root = document.querySelector('[data-task-grid-root]')
    if (!root) return filteredTasks.map((task) => task.id)

    const ids = Array.from(root.querySelectorAll<HTMLElement>('.task-card-wrapper[data-task-id]'))
      .map((card) => card.dataset.taskId)
      .filter((id): id is string => Boolean(id))
    return ids.length ? ids : filteredTasks.map((task) => task.id)
  }, [filteredTasks])

  const handleSelectAllVisibleTasks = useCallback(() => {
    setSelectedTaskIds(getVisibleTaskIds())
  }, [getVisibleTaskIds, setSelectedTaskIds])
  ...
  const updateInputBarClearance = useCallback((force = false) => {
    if (!force && inputBarTransitioningRef.current) return
    if (inputBarClearanceFrameRef.current !== null) {
      if (!force) return
      window.cancelAnimationFrame(inputBarClearanceFrameRef.current)
    }
    inputBarClearanceFrameRef.current = window.requestAnimationFrame(() => {
      ...
      const nextClearance = Math.ceil(clearance)
      if (!force && Math.abs(nextClearance - lastInputBarClearanceRef.current) < 2) return
      lastInputBarClearanceRef.current = nextClearance
      document.documentElement.style.setProperty('--input-bar-clearance', `${nextClearance}px`)
    })
  }, [])

  const cancelInputBarClearanceFrame = useCallback(() => {
  ...
  useLayoutEffect(() => {
    ...
    updateInputBarClearance(true)
    const handleInputBarClearanceChange = () => updateInputBarClearance()
    const observer = new ResizeObserver(handleInputBarClearanceChange)
  ...
  useEffect(() => {
    const handlePointerDown = (event: PointerEvent) => {
      ...
      if (!bar || !(target instanceof Node)) return
  ...
  return (
    <>
      ...
      <div data-input-bar className={`input-bar-shell ${inputBarExpanded ? 'is-expanded' : ''} ${inputBarOverflowReady ? 'is-overflow-ready' : ''} fixed bottom-4 sm:bottom-6 left-1/2 -translate-x-1/2 z-30 w-full max-w-4xl px-3 sm:px-4 transition-all duration-300`}>
        <InputBatchBars
        ...
        />
        <div ref={cardRef} className="compact-input-card liquid-glass-inputbar bg-white/70 dark:bg-gray-900/70 backdrop-blur-2xl border border-white/50 dark:border-white/[0.08] shadow-[0_8px_30px_rgb(0,0,0,0.08)] dark:shadow-[0_8px_30px_rgb(0,0,0,0.3)] rounded-2xl sm:rounded-3xl p-3 sm:p-4 ring-1 ring-black/5 dark:ring-white/10">
          {/* 移动端拖动条 */}
          <div
            ref={handleRef}
          ...
          {/* 参数 + 按钮 */}
          <div className="input-expanded-section mt-3">
            {/* 桌面端布局 */}
