...
import { clearFailedTasks, useStore, taskMatchesFavoriteScope, taskMatchesFilterStatus, taskMatchesSearchQuery } from '../store'
...
export default function SearchBar() {
  ...
  const failedCount = useStore((s) => {
    ...
    return s.tasks.filter((task) => {
      ...
      if (!taskMatchesFavoriteScope(task, s.filterFavorite, s.activeFavoriteCollectionId)) return false
  ...
  const handleClearFailed = () => {
    ...
    const failedTaskIds = state.tasks
      .filter((task) => {
        if (!taskMatchesFilterStatus(task, 'error')) return false
        if (!taskMatchesFavoriteScope(task, state.filterFavorite, state.activeFavoriteCollectionId)) return false
        return taskMatchesSearchQuery(task, q)
      })
      .map((task) => task.id)
