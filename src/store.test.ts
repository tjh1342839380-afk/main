...
describe('agent context for removed outputs', () => {
  ...
  it('injects PPT Master analysis while preserving the original input_file', async () => {
    const file = {
      id: 'agent-file-analyzed-pptx',
      name: '产品模板.pptx',
      mimeType: 'application/vnd.openxmlformats-officedocument.presentationml.presentation',
      size: 12,
    }
    ...
    const fetchMock = vi.spyOn(globalThis, 'fetch').mockResolvedValue(new Response(JSON.stringify({
      ...
      slide_count: 2,
      analysis_text: 'slide 1 slot s01_sh2 role=title',
    ...
    try {
      ...
      expect(serializedInput).toContain('input_file')
      expect(serializedInput).toContain('<ppt_master_analysis')
      expect(serializedInput).toContain('file_id=\\"agent-file-analyzed-pptx\\"')
      expect(serializedInput).toContain('slot s01_sh2')
      expect(fetchMock).toHaveBeenCalledWith('https://ppt.example.com/v1/analyze', expect.objectContaining({
        method: 'POST',
  ...
  it('stores PPT Master template-fill output as a downloadable Agent file', async () => {
    const file = {
      id: 'agent-file-fill-pptx',
      name: '品牌模板.pptx',
      mimeType: 'application/vnd.openxmlformats-officedocument.presentationml.presentation',
      size: 12,
    }
    await putAgentReferenceFile({
      ...
      pptMasterAnalysis: {
        ...
        analyzedAt: 1,
        slideCount: 1,
        text: 'slide 1 slot s01_sh2 role=title',
      },
    })
    useStore.setState((state) => ({
      settings: normalizeSettings({
        ...
        outputItems: [{
          ...
          arguments: JSON.stringify({
            ...
            slides: [{
              source_slide: 1,
              purpose: '封面',
              layout_pattern: '主标题封面',
              why_fit: '适合作为开场',
              risk: '标题保持简短',
              notes: null,
              transition: 'fade',
              replacements: [{ slot_id: 's01_sh2', text: '品牌提案' }],
              table_edits: [],
              chart_edits: [],
            }],
          }),
        }],
        responseId: 'response-fill-1',
      })
      .mockResolvedValueOnce({
        text: '原生可编辑 PPTX 已生成。',
        images: [],
        outputItems: [{ type: 'message', content: [{ type: 'output_text', text: '原生可编辑 PPTX 已生成。' }] }],
        responseId: 'response-fill-2',
      })
    ...
    try {
      ...
      const round = useStore.getState().agentConversations[0].rounds.at(-1)
      expect(round?.outputFiles).toHaveLength(1)
      expect(round?.outputFiles?.[0]).toMatchObject({
        name: '品牌提案.pptx',
        mimeType: 'application/vnd.openxmlformats-officedocument.presentationml.presentation',
        size: 4,
      })
      expect(JSON.stringify(round?.responseOutput)).toContain('PPT Master v4.1.0')
      expect(fetchMock).toHaveBeenCalledWith('https://ppt.example.com/v1/fill', expect.objectContaining({ method: 'POST' }))
    } finally {
      fetchMock.mockRestore()
    }
  })
